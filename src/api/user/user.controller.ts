...
export default class UserController {
    ...
    public async updateWeight(req: Request, res: Response): Promise<Response> {
        const userId = req.user.id;
        const { weight, date } = req.body;
        try {
            const result = await UserService.updateWeight(userId, weight, date);
            if (!result) {
                return res.status(404).json({ message: 'User not found' });
            }
            return res
                .status(200)
                .json({ message: 'Update weight successfully' });
        } catch (err) {
            console.error(err);
            return res.status(500).json({ message: 'Server error' });
        }
